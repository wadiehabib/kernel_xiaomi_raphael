...
#define NSS_SWITCH_TPUT_THRESH_1  250 //200Mbps
#define NSS_SWITCH_TPUT_THRESH_2  150 //100Mbps
...
#define NSS_SWITCH_2T1_MIN_INTERVAL 5  //5s, nss2->1 slow
#define NSS_SWITCH_1T2_MIN_INTERVAL 2  //2s, nss1->2 fast
...
void wlan_hdd_init_dynamic_nss_ctx(struct hdd_adapter *adapter, struct dynamic_nss_context *dynamic_nss) {
	...
	dynamic_nss->nss1t2_switch_flag = true;

	if ((sta_ctx != NULL) && hdd_conn_is_connected(sta_ctx)) {
		...
		hdd_debug("flag = %d, rssi_th = %d, tput_th = %d, intf_name = %s",
			dynamic_nss->nss1t2_switch_flag, dynamic_nss->rssi_th, dynamic_nss->tput_th, adapter->dev->name);
	}
}
...
void wlan_hdd_start_dynamic_nss(struct hdd_adapter *adapter)
{
	...
	if (s_config_dynamic_nss && (adapter->device_mode == QDF_STA_MODE)
		...
		//step 2: start timer
		status = qdf_mc_timer_start(&dynamic_nss->tput_stats_timer, 1000); //interval: 1s
		if (QDF_IS_STATUS_ERROR(status)) {
...
static void wlan_hdd_do_nss_switch(struct hdd_adapter *adapter, struct dynamic_nss_context *dynamic_nss) {
	uint32_t tput = 0;
	...
	hdd_debug("concur = %d, rssi = %d, tput = %d", concur_mode, adapter->rssi, tput);
	...
	if (!concur_mode) {
		if (adapter->rssi >= dynamic_nss->rssi_th) {
			...
			if (tput >= dynamic_nss->tput_th) {
				...
				if (!dynamic_nss->nss1t2_switch_flag) {
					dynamic_nss->consct_high_tput_cnt++;
					if (dynamic_nss->consct_high_tput_cnt == NSS_SWITCH_1T2_MIN_INTERVAL) {
						//set 2x2 mode
						hdd_debug("tput_stats_timer: nss=2, high tput, intf_name = %s", adapter->dev->name);
						wlan_hdd_set_nss_and_antenna_mode(adapter, NSS_2x2_MODE, HDD_ANTENNA_MODE_2X2);
				...
				if (dynamic_nss->nss1t2_switch_flag) {
					...
					if (dynamic_nss->consct_low_tput_cnt == NSS_SWITCH_2T1_MIN_INTERVAL) {
						//set 1x1 mode
						hdd_debug("tput_stats_timer: nss=1, low tput, intf_name = %s", adapter->dev->name);
						wlan_hdd_set_nss_and_antenna_mode(adapter, NSS_1x1_MODE, HDD_ANTENNA_MODE_1X1);

						dynamic_nss->nss1t2_switch_flag = false;
						dynamic_nss->consct_low_tput_cnt = 0;
					}
				}
			}
			...
			if ((!dynamic_nss->nss1t2_switch_flag) && (dynamic_nss->consct_low_rssi_cnt == NSS_SWITCH_1T2_MIN_INTERVAL)) {
				hdd_debug("tput_stats_timer: nss=2, low rssi, intf_name = %s", adapter->dev->name);
				wlan_hdd_set_nss_and_antenna_mode(adapter, NSS_2x2_MODE, HDD_ANTENNA_MODE_2X2);
		...
		if (!dynamic_nss->nss1t2_switch_flag) {
			hdd_debug("tput_stats_timer: nss=2, coex, intf_name = %s", adapter->dev->name);
			wlan_hdd_set_nss_and_antenna_mode(adapter, NSS_2x2_MODE, HDD_ANTENNA_MODE_2X2);
			dynamic_nss->nss1t2_switch_flag = true;
		}
	}
...
void wlan_hdd_check_if_nss_switch(void *data)
{
	...
	wlan_hdd_do_nss_switch(adapter, dynamic_nss);
	hdd_exit();
}
